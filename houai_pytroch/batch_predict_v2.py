"""
批量预测脚本（V2）：用 continue_train 产出的 best_model.pth 对全部数据集图片生成
绿线(真实) + 红线(预测) 叠加图，保存到 result_v2/
"""
import os, sys
import cv2
import numpy as np
...
MODEL_PATH = os.path.join(BASE_DIR, 'venv', 'model_pytorch_v2', 'best_model.pth')
...
RESULT_DIR = os.path.join(BASE_DIR, 'result_v2')
os.makedirs(RESULT_DIR, exist_ok=True)


# ============================================================
# 模型定义（标准 UNet：Conv→BN→ReLU，与 unet_pytorch / continue_train 一致）
# ============================================================
